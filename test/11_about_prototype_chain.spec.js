...
describe.skip("About Prototype Chain (topics/11_about_prototype_chain.js)", function() {
  let father = {
    b: 3,
    c: 4
  };
  ...
  it("Who has the masteredJs property?", function() {
    ...
    assert.equal(
      __,
      student.masteredJs,
      "did the regular student mastered js?"
    );
  });
  ...
  it("Separate proto", function() {
    ...
    assert.equal(antonio.technologies.length, __, "What has Antonio learned?");
    assert.equal(
